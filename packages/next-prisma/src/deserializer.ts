...
function valueIs(value: any, types: any[]) {
  return types
    .map((type) => type.name.toLowerCase() == typeof value)
    .includes(true);
}
...
function renderAttributes(field: WritableDeep<DMMF.Field>): string {
  const {
    relationFromFields,
    relationToFields,
    relationName,
...
function renderModelFields(fields: WritableDeep<DMMF.Field>[]): string[] {
...
function renderPreviewFeatures(
  previewFeatures: GeneratorConfig['previewFeatures']
): string {
  return previewFeatures.length
...
function deserializeModel(model: WritableDeep<DMMF.Model>): string {
  const {
    name,
    fields,
    dbName,
...
function deserializeEnum({
  ...
  return renderBlock(
    'enum',
    name,
    [...outputValues, renderDbName(dbName || null)],
    documentation
  );
}
...
export async function deserializeGenerators(generators: GeneratorConfig[]) {
  return generators
    .map((generator) => deserializeGenerator(generator))
    .join('\n');
}
